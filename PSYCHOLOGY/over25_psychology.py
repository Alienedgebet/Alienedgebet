...
# --- 2. DYNAMIC PATHS FOR SERVERS (Fixing the File Not Found Bug) ---
...
# ==============================================================================
def run_o25_psychology_engine(target_date=None):
    """
    Executes the AlienEdge Over 2.5 Psychology Engine.
    Reads from BOTH Stage 1 (Base Math) and Stage 2 (Premium Council).
    Applies strict forensic filters to find "Value Bets" across both tiers.
    """
    ...
    logging.basicConfig(level=logging.INFO, format='%(asctime)s -[O2.5 ENGINE] - %(message)s')
    ...
    # ==============================================================================
    # 📊 LEAGUE STANDINGS (FOR RELEGATION DESPERATION)
    # ==============================================================================
    ...
    # ==============================================================================
    # 📊 MATCH DATA EXTRACTORS (FULL TIME, HALF TIME, STATS)
    # ==============================================================================
    ...
    # ==============================================================================
    # 🧠 OVER 2.5 TACTICS ENGINE (The Chaos Profiler)
    # ==============================================================================

    def analyze_o25_tactics(team_id, current_match_id):
        ...
        # Check for Clean Sheets (0 clean sheets = No Defense)
        ...
        return {
            ...
            "is_ht_active": is_ht_active,
            "is_ht_killer": is_ht_killer,
            "is_ht_vulnerable": is_ht_vulnerable,
            "is_clinical": is_clinical,
            "total_goals": total_goals,
            "no_clean_sheets": no_clean_sheets,
            "is_wounded": is_wounded,
            "avg_sot": round(avg_sot, 1)
        }

    # ==============================================================================
    # ⚖️ THE ALIENEDGE OVER 2.5 RULES ENGINE (Reverse Psychology)
    # ==============================================================================

    def calculate_o25_score(h, a, h_spear, a_spear, h_pos, a_pos):
        """Calculates the 100-Point Over 2.5 Score based on your exact Rules."""
        score = 0
        reasons =[]
        ...
        if both_attacking and both_glass_jaw and both_ht_active:
            ...
            # If not the Holy Grail, assign individual points
            if both_attacking:
        ...
        # ---------------------------------------------------------
        # 🧠 THE NEW PSYCHOLOGY ADDITIONS
        # ---------------------------------------------------------
        # BTTS Synergy (Underdog probability very high)
        if ud_spear > 70.0:
            score += 10; reasons.append("🤝 BTTS Synergy / High UD (+10)")
            
        # Wounded Favorite Rage
    ...
    # 🚀 MAIN EXECUTION
    # ==============================================================================
    print(f"\n>> [AlienEdge OVER 2.5 ENGINE] Initializing Reverse Psychology Protocol... ({TODAY_STR})")
    
    # 1. 🛠️ SURGICAL FIX: READ BOTH STAGE 1 AND STAGE 2
    raw_data_dict = {}
    
    for file_path in [STAGE1_FILE, STAGE2_FILE]:
        if os.path.exists(file_path):
            try:
                ...
                # Grade/Votes logic matches perfectly
                ...
                for _, row in df_in.iterrows():
                    ...
                    # Dictionary guarantees NO duplicates if match appears in both stages
                    raw_data_dict[key] = {
            ...
            print(f"⚠️ [WARNING] {os.path.basename(file_path)} not found.")

    raw_data = list(raw_data_dict.values())
    
    if not raw_data:
        print("🛑 FATAL ERROR: No matches found in Stage 1 or Stage 2. Please run base engines first.")
        return[]

    # 2. READ CATALYST FOR UD/FAV SPEARS
    cat_db = {}
    ...
    # 3. UNLIMITED PAGINATION TO GET TEAM IDs & LEAGUE INFO
    todays_fixtures_map = {}
    print("   🔍 Fetching API IDs and League Data (Unlimited Pagination)...")
    page = 1
    while True:
    ...
    for match in raw_data:
        try:
            ...
            # Fetch Standings for Relegation Desperation
            pos_map = get_league_standings_map(lid, sid)
            h_pos = pos_map.get(int(home_id), 99)
            a_pos = pos_map.get(int(away_id), 99)

            # Analyze Tactics
            h_traits = analyze_o25_tactics(home_id, fid)
            a_traits = analyze_o25_tactics(away_id, fid)
            
            # Calculate Over 2.5 Score
            o25_score, reasons = calculate_o25_score(h_traits, a_traits, h_spear, a_spear, h_pos, a_pos)

            # Classify Tier
            if o25_score >= 60: tier = "💎 DIAMOND OVER 2.5"
            elif o25_score >= 40: tier = "🔥 SOLID OVER 2.5"
            elif o25_score >= 25: tier = "📊 PLAYABLE OVER"
            ...
            print(f" > Audited {fix_name[:35]:<35} | Score: {o25_score:>3}/100 | {tier}")
            
        except Exception as e:
            continue

    # ==============================================================================
    # 🏆 FINAL ALIENEDGE OVER 2.5 PRINTS
    # ==============================================================================
    ...
    print("\n" + "💎"*45)
    print(" 💎 ALIENEDGE VERIFIED LOCKS: OVER 2.5 GOALS 💎")
    print("💎"*45)
    ...
    if not diamond_df.empty:
        for i, (_, row) in enumerate(diamond_df.iterrows(), 1):
            print(f"{i}. {row['Fixture']} -> 🏆 BET: OVER 2.5 (Score: {row['Score']}/100)")
    ...
    print("\n" + "🔥"*45)
    print(" 🔥 SOLID VERIFIED WINS: OVER 2.5 GOALS 🔥")
    print("🔥"*45)
    ...
    print("\n" + "="*120)
    print("📊 FULL OVER 2.5 FORENSIC BOARD")
    print("="*120)
    ...
    print(f"\n💾 [Done] AlienEdge Over 2.5 Predictions saved to: {OUTPUT_CSV}")
...
def run_over25_aggregator(target_date=None):
    return run_o25_psychology_engine(target_date)

if __name__ == "__main__":
    pd.set_option('display.max_rows', None)
    pd.set_option('display.width', 1000)
    run_o25_psychology_engine()