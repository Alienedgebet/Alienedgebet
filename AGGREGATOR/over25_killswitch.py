...
# ==============================================================================
def run_over25_stage3(target_date):
    """
    Executes Over 2.5 Engine Stage 3 (The Handshake / Premium Killswitch).
    Math, strict rules, and thresholds are 100% untouched.
    Now strictly reads ONLY from Stage 2 (The Premium Council).
    """
    ...
    def extract_final_score_string(scores):
        """Returns the final score (e.g. '2-1') Fixed for Sportmonks V3"""
        home, away = 0, 0
        found = False
        for entry in (scores or[]):
            if not isinstance(entry, dict): continue
            
            # FIX: Handle Sportmonks v3 nested dictionary structure properly
    ...
    # -------------------------
    print("\n--- FINAL AGGREGATOR (STRICT H2H KEPT / PARSER FIXED) ---")
    
    all_picks = {}
    
    # 🛠️ SURGICAL FIX: Loading strictly from Stage 2
            ...
            if not df.empty:
                ...
                if id_col:
                    for _, row in df.iterrows():
                        ...
                        all_picks[fid]["strategies"].append("9_Layer_Council")
        except Exception as e: 
            print(f"Error reading {STAGE2_FILE}: {e}")
    else:
        print(f"[!] Warning: {STAGE2_FILE} not found. Ensure Stage 2 ran successfully.")
    ...
    results =[]
    # Uses the injected target_date to match the engines
    ...
    for fid in unique_ids:
        ...
        if not passed:
            ...
            print(f"   Reason: {count}/5 Overs in H2H History. (PASSED STRICT RULE)")
            print(f"   Scores: {history}")
            print("-" * 30)
        # ----------------------------------------------------
    ...
    if results:
        ...
        cols =["Match", "Poisson%", "Grade", "H2H_Record", "Odds", "Failures"]
        ...
        print("\n" + "="*80)
        print(f"FINAL VERIFIED LIST ({target_date})")
        print("="*80)
        ...
        print("\n[O2.5 Stage 3] No matches passed the strict criteria.")
        return