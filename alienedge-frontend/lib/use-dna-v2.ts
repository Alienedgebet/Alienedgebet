 ...
 * `/api/dna/v2/{date}` and can NEVER silently show today's DNA. Pass
 * `dateOverride` (the DNA Analysis page passes its `?date=` URL param) to
 * override the global selection.
 *
 * Uses the disk-only `/api/dna/v2/{date}` endpoint (no engine recompute)
...
export function useDnaV2(dateOverride?: string): UseApiResult<DnaV2Response> {
  ...
  return useApi(() => dnaV2Api.get(date), [date], {
    cacheKey: `dna-v2:${date}`,
