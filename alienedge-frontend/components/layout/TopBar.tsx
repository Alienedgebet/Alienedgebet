...
export function TopBar() {
  ...
  return (
    ...
    >
      {/* Top row — platform identity (always visible, centred) */}
      <div className="flex w-full items-center justify-center py-1.5 border-b border-border/40">
        <span className="select-none text-2xs font-medium uppercase tracking-[0.15em] text-text-dim">
          Football Intelligence Platform
        </span>
      </div>

      {/* Bottom row — hamburger · date selector · status · avatar */}
      <div className="flex items-center justify-between gap-2 px-4 py-2 md:px-6">
        ...
        {/* Global date selector — drives every {date} endpoint */}
        <DateSelector />

        {/* Right — API status + War Room toggle + user avatar */}
        <div className="flex items-center gap-2">
          {/* API status pill */}
          <div
            className={cn(
              "flex items-center gap-1.5 rounded border px-2 py-1 font-mono text-2xs transition-colors",
              status === "online"
          ...
          {/* War Room / Leaderboard drawer toggle — mobile only */}
          <button
            type="button"
            onClick={() => {
              closeSidebar();
